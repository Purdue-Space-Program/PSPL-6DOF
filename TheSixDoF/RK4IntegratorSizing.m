function [out, mach, AoA, accel] = RK4IntegratorSizing(time, input, rasData, totCoM, totMass, J, wind, params)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% PSP FLIGHT DYNAMICS:
%
% Title: RK4Integrator
...
finSpan = .15;                                                      % fin height [m]
finThickness = 0.16 / 39.37;                                        % thickness of the fins [m]
finCount = 3;                                                       % number of fins on the rocket [-]
rocketOD = 8.625 / 39.37;                                           % outer diameter of the rocket [m]
A = pi * (rocketOD / 2)^2 + finCount * (finThickness * finSpan);    % rocket cross-sectional area [m^2]
mInit = 147.018359;                                                 % wet mass of the rocket [kg]
massFlow = 4.056023;                                                % mass flow rate [kg/s]
thrustMag = 7932.629233;                                            % average thrust of rocket [N]
burnTime = 15.185;                                                  % burn time [sec]
bodyVector = [1;0;0];                                               % vector in the body axis running through the nose
ExitA = 0.0100541734;                                               % exit area of the nozzle [m^2]
ExitP = 101352.9;                                                   % exit pressure of the nozzle [Pa]
radius = rocketOD / 2;                                              % radius of rocket [m]
