...
if (get(hObject,'Value') == 1 || get(hObject,'Value') == 2)
    ...
    set(handles.editSaveFilename,'String','NACA.dat');
...
% Force XFOIL ordering for .dat files (TE->LE upper, then LE->TE lower)
[~,~,ext] = fileparts(saveFilename);
if strcmpi(ext,'.dat')
    popSaveType = 2;   % TE->LE mode ensures CCW order around the airfoil
end
...
if (popSaveType == 1 || popSaveType == 2)
    ...
    [pth,nam,ext] = fileparts(saveFilename);
    if isempty(ext) || ~strcmpi(ext,'.dat')
        saveFilename = fullfile(pth, [nam '.dat']);
    end

    % ----- Open and write XFOIL-style title line (no column headers) -----
    fid = fopen(saveFilename,'wt');
    typeNACA = evalin('base','typeNACA');
    fprintf(fid,'NACA %s\n', typeNACA);

    % ===== ENFORCE XFOIL ORDER: start at TE, go CCW =====
    % Upper surface must run TE -> LE  (x decreasing)
    if xu(1) < xu(end)          % currently LE->TE? flip it
        xu = flipud(xu);
        yu = flipud(yu);
    end

    % Lower surface must run LE -> TE  (x increasing)
    if xl(1) > xl(end)          % currently TE->LE? flip it
        xl = flipud(xl);
        yl = flipud(yl);
    end

    % Write upper surface: TE -> LE (all points)
    for i = 1:gridPts
        fprintf(fid,'%2.6f %2.6f\n', xu(i), yu(i));
    end

    % Decide whether TE is closed (avoid duplicate identical TE point)
    % After the enforcement above:
    %   - upper(1) is TE_upper, upper(end) is LE
    %   - lower(1) is LE,       lower(end) is TE_lower
    TE_closed = (abs(xu(1) - xl(end)) < 1e-9) && (abs(yu(1) - yl(end)) < 1e-9);

    % Write lower surface: LE -> TE (skip LE duplicate; optionally skip TE duplicate)
    iStart = 2;                          % skip LE (already written at end of upper)
    iEnd   = gridPts - (TE_closed);      % skip TE if closed to avoid zero-length panel
    for i = iStart:iEnd
        fprintf(fid,'%2.6f %2.6f\n', xl(i), yl(i));
    end
